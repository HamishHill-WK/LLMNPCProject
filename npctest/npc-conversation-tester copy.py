    ...
    def get_npc_response(self, npc_id: str, message: str, context: Dict[str, Any]) -> str:
        ...
        if 'conversation_history' in context and context['conversation_history']:
            ...
            for exchange in recent_history:
                ...
                # Include thought component only if this is the current speaker
                if 'thought' in exchange and speaker == npc_id:
                    thought = exchange.get('thought', '')
                    if thought:
                        context_desc += f"{speaker} (thinking): {thought}\n"
                        
                context_desc += f"{speaker}: \"{content}\"\n"
