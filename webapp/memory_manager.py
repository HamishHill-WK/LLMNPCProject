...
class MemoryManager:
    def __init__(self, max_short_term=3, max_long_term = 3, characters={}, knowledge_engine=None):
    ...
    def add_interaction(self, character_id, other_id, other_message, character_response, chain_of_thought, location):
        ...
        dialogue = character_response.replace('<character_response>', '').replace('</character_response>', '').replace(f"{self.characters[character_id]['name']}: ", "").strip()
    ...
    def summarize_long_term(self, character_id):
        ...
        old_summary = self.memories[character_id]["summary"]
...
Resolve any contradictions by favoring newer information.
        ...
        response = om.clean_response(response)
